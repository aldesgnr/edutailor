editor.once('load', function () {
    ...
    var createLinesModel = function (app) {
        ...
        var vertexFormat = new pc.VertexFormat(app.graphicsDevice, [{ semantic: pc.SEMANTIC_POSITION, components: 3, type: pc.TYPE_FLOAT32 }])
        ...
        var meshInstances = []
        var materials = [createMaterial(new pc.Color(1, 0, 0, 1.1)), createMaterial(new pc.Color(0, 1, 0, 1.1)), createMaterial(new pc.Color(0, 0, 1, 1.1))]
    ...
    editor.method('gizmo:rotate:visible', function (state) {
        ...
        for (let i = 0; i < gizmo.hoverable.length; i++) {
            if (!gizmo.hoverable[i].model) continue
    ...
    editor.method('gizmo:rotate:position', function (x, y, z) {
        if (x === undefined) return gizmo.root.getPosition()

        gizmo.root.setPosition(x, y, z)

        if (gizmo.root.enabled) editor.call('viewport:render')
    })
    ...
    editor.once('viewport:load', (app) => {
        ...
        editor.on('viewport:pick:hover', function (node, picked) {
            ...
            if (hover) {
                if (node.axis && hoverAxis !== node.axis) {
                    // set normal material
                    if (hoverAxis) gizmo.line[hoverAxis].material = gizmo.line[hoverAxis].mat

                    if (!hoverAxis && !evtTapStart) evtTapStart = editor.on('viewport:tap:start', onTapStart)
        ...
        editor.on('viewport:postUpdate', function (dt) {
            if (gizmo.root.enabled) {
                ...
                quat.copy(gizmo.root.getRotation()).invert().transformVector(vecA, vecA)
